  ...
  return (
    ...
    >
      <div style={{ background: "#fff", borderRadius: 16, width: 960, maxWidth: "calc(100vw - 32px)", maxHeight: "90vh", overflow: "auto", boxShadow: "0 24px 64px rgba(0,0,0,.2)" }}>
        ...
        {/* ── Step: choose mode ── */}
        {step === "choose" && (
          <div className="cv-modal-choose-grid" style={{ padding: "20px 24px 24px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, maxWidth: 640, margin: "0 auto" }}>
            <ModeCard
              icon="✦"
      ...
      <style>{`@media (max-width: 420px) { .cv-modal-choose-grid { grid-template-columns: 1fr !important; } }`}</style>
    </div>
  );
}
