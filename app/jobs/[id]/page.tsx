...
function SaveApplyButton({ job }: { job: any }) {
  ...
  const isDirectJob = !job.original_url
  ...
  // Save application to DB then optionally open external URL
  ...
  // Handle click on main apply button
  const handleApply = async () => {
    ...
    if (isDirectJob) {
      // Show on-platform application form
      setShowForm(true);
      return;
    }
    // External job: save + open URL
    setState("saving");
  ...
  // Submit on-platform application form
  const submitForm = async () => {
    ...
    try {
      ...
      // 2. Send email via a simple mailto (or you can wire to an API route / n8n webhook)
...
— Envoyé via TalentMaroc`
      ...
      // Open mailto so candidate's email client sends it
      // The recruiter email isn't stored yet — for now we open a pre-filled mailto
      // In a future iteration, store employer email and POST to /api/send-application
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <div>
              ...
              <input value={formData.name} onChange={e=>setFormData(p=>({...p,name:e.target.value}))}
                ...
                style={{ border:"1.5px solid #e5e7eb", borderRadius:8, padding:"9px 12px", width:"100%", fontSize:13, fontFamily:"inherit", color:"#0f172a", background:"white", outline:"none" }}/>
            </div>
            <div>
            ...
            <div>
              ...
              <textarea value={formData.message} onChange={e=>setFormData(p=>({...p,message:e.target.value}))}
                ...
                style={{ border:"1.5px solid #e5e7eb", borderRadius:8, padding:"9px 12px", width:"100%", fontSize:13, fontFamily:"inherit", color:"#0f172a", background:"white", outline:"none", resize:"vertical", lineHeight:1.6 }}/>
            </div>
      ...
      {/* ── MAIN APPLY BUTTON ── */}
      {!formSent ? (
        <button onClick={handleApply} disabled={state==="saving"||state==="saved"} className="cta-btn"
          style={{ cursor:state==="saved"?"default":"pointer", opacity:state==="saving"?0.7:1 }}>
          {state==="saving" ? "Envoi en cours…"
            : state==="saved" ? "✓ Candidature envoyée !"
            : isDirectJob ? (authed ? "📋 Postuler à cette offre" : "🔐 Connexion pour postuler")
      ...
      {/* ── SAVE & TRACK (for external jobs, shown separately) ── */}
      {!isDirectJob && !formSent && (
        <button onClick={handleApply} disabled={state==="saving"||state==="saved"}
          style={{ ...btnStyle,
          ...
          {state==="saving" ? "Sauvegarde…" : state==="saved" ? "✓ Sauvegardée dans votre dashboard" : state==="error" ? "Erreur — réessayez" : authed ? "📋 Sauvegarder & suivre" : "🔐 Connexion pour sauvegarder"}
        </button>
      )}
